...
class CallbackReportingModelSummary(callback.Callback):
    ...
    def __call__(self, options, history, model, losses, outputs, datasets, datasets_infos, callbacks_per_batch,
        ...
        for name, layer in summary.items():
            ...
            input_shape.append(str(layer['input_shape']))
            output_shape.append(str(layer['output_shape']))
            nb_params.append(str(layer['nb_params']))
            nb_trainable_params.append(str(layer['total_trainable_params']))
        ...
        export_table(options, 'model_summary',  batch, table_role='data_tabular', clear_existing_data=True,
