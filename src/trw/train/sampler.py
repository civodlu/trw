...
class SamplerSequential(Sampler):
    ...
    def __iter__(self):
        if self.batch_size == 1:
            return iter(range(utils.len_batch(self.data_source)))
        else:
            return _SamplerSequentialIter(utils.len_batch(self.data_source), self.batch_size)
...
class SamplerRandom(Sampler):
    ...
    def initializer(self, data_source):
        ...
        self.num_samples = utils.len_batch(self.data_source)
...
class SamplerClassResampling(Sampler):
    ...
    def initializer(self, data_source):
        ...
        data_source_samples = utils.len_batch(data_source)

        classes = utils.to_value(data_source[self.class_name])  # we want numpy values here!
